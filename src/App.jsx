...
export default function App() {
    ...
    // addColBtnRef used to anchor the portal popover below the button
    const addColBtnRef = useRef();
    // portal popover position (set when button is clicked)
    ...
    // use data-addcol-popover attr instead of a ref bc the popover is in a portal
    useEffect(() => {
        if (!showAddCol) return;
        const handler = e => {
            if (addColBtnRef.current?.contains(e.target)) return;
            if (!e.target.closest("[data-addcol-popover]")) setShowAddCol(false);
        };
    ...
    // deleted column - jobs move back to 'watchlist'
    ...
    // open add-column popover: measure button position so the portal renders in the right place
    const openAddCol = () => {
    ...
    return (
        <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh", background: "var(--bg-page)" }}>
            ...
            >
                ...
                {/* nav - text labels collapse to icons below 880px */}
                <nav aria-label="Main navigation" style={{ display: "flex", gap: 2, flexShrink: 0 }}>
                    {NAV.map(n => (
                        <button key={n.id} onClick={() => setView(n.id)} aria-current={view === n.id ? "page" : undefined}
                            ...
                            {/* icon always visible - color: inherit so it follows the button's text color */}
                            <span aria-hidden="true" style={{ display: "flex", alignItems: "center", color: "inherit" }}>
                                {n.id === "board" && <NavIconBoard />}
                                {n.id === "table" && <NavIconTable />}
                                {n.id === "stats" && <NavIconStats />}
                            </span>
                            {/* label hidden below 880px */}
                            <span className="btn-text">{n.label}</span>
                        </button>
                    ))}
                </nav>

                {/* divider between nav and primary actions */}
                <div aria-hidden="true" style={{ width: 1, height: 20, background: "var(--border-default)", flexShrink: 0 }} />
                {/* STATS */}
                ...
                {/* quick add column - popover rendered via portal below */}
                <button
                    ref={addColBtnRef}
                    onClick={openAddCol}
                    ...
                    title="Add column"
                    style={{ ...btnStyle("outline"), padding: "6px 10px", fontSize: 13, flexShrink: 0 }}
                >
                    <span className="btn-icon" aria-hidden="true" style={{ alignItems: "center", color: "inherit" }}>
                        <IconAddCol />
                    </span>
                    <span className="btn-text">+ Col</span>
                </button>
                {/* add job */}
                <button onClick={() => setModal({ mode: "add", column: "watchlist" })} style={{ ...btnStyle("primary"), flexShrink: 0 }} aria-label="Add a new job">
                ...
                {/* import/export/settings - labels hidden below 900px, hidden entirely below 480px */}
                {/* Import */}
                <button onClick={handleImportClick} style={btnStyle("ghost")} aria-label="Import backup file" className="header-import">
                    <span aria-hidden="true" style={{ display: "flex", alignItems: "center", color: "inherit" }}><IconImport /></span>
                    <span className="btn-text-secondary">Import</span>
                </button>
                {/* Export */}
                <button onClick={handleExport} style={btnStyle("ghost")} aria-label="Export backup" className="header-export">
                    <span aria-hidden="true" style={{ display: "flex", alignItems: "center", color: "inherit" }}><IconExport /></span>
                    <span className="btn-text-secondary">Export</span>
                </button>
                ...
                <button onClick={() => setShowSettings(v => !v)} style={btnStyle("ghost")} aria-label="Open settings" aria-expanded={showSettings}>
                    <span aria-hidden="true" style={{ display: "flex", alignItems: "center", color: "inherit" }}><IconSettings /></span>
                    <span className="btn-text-secondary">Settings</span>
                </button>
            </header>
            {/* add-column popover - portal into document.body so it's not clipped by the header */}
            {showAddCol && createPortal(
                <div
                    data-addcol-popover
                    style={{
                        position: "fixed",
...
function IconAddJob() {
    return (
        ...
        // </svg>
        <svg height="15px" width="15px" viewBox="0 0 246 471.1" fill="currentColor">
